...
async function step9Guard(ctx) {
    ...
    if (!formatted.trim())
        throw new Error("Step 9 failed: missing formatted output");
    await (0, ops_utils_1.pause)(emit, { step: 9, message: "Final guard engaged. Running profanity, bias, and hallucination heuristics." });
    const flags = [];
    const prof = profanityHits(formatted);
    if (prof.length)
        flags.push({ type: "profanity", hits: prof });
    const bias = biasHits(formatted);
    if (bias.length)
        flags.push({ type: "bias_keywords", hits: bias });
    const risk = hallucinationRisk(formatted);
    if (risk >= 0.55)
        flags.push({ type: "hallucination_risk", score: risk });
    await (0, ops_utils_1.pause)(emit, { step: 9, message: `Guard analysis complete. Risk ${(risk || 0).toFixed(2)}. Flags ${flags.length}.` });
...
exports.default = step9Guard;
//# sourceMappingURL=step9_guard.js.map