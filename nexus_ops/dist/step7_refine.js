"use strict";
/**
 * NEXUS PRO V4 - Step 7: Refine
 * Quantum refine with verb strengthening and tone adjustment
 */
...
async function step7Refine(ctx) {
    ...
    if (!verified.trim())
        throw new Error("Step 7 failed: missing verified draft");
    await (0, ops_utils_1.pause)(emit, { step: 7, message: "Quantum refine engaged. Strengthening verbs and enforcing active voice." });
    let out = strengthenVerbs(verified);
    await (0, ops_utils_1.pause)(emit, { step: 7, message: "Running tone calibration and whitespace normalization." });
    out = enforceActiveVoice(out);
    out = adjustTone(out, ctx?.userQuery);
    out = normalizeWhitespace(out);
    return out;
}
exports.default = step7Refine;
//# sourceMappingURL=step7_refine.js.map