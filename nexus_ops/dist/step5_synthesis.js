...
function agentWeight(agent) {
    if (agent === "reasoner")
        return 1.15;
    if (agent === "math_code_wizard")
        return 1.12;
    if (agent === "context_king")
        return 1.05;
    if (agent === "generalist")
        return 1.0;
    if (agent === "efficient_backup")
        return 0.9;
    return 1.0;
}
...
function clusterKey(fact) {
    const words = normalizeKey(fact).split(" ").filter(Boolean);
    return words.slice(0, 7).join(" ");
}
function summarizeFacts(facts, maxChars) {
    const lines = [];
    let used = 0;
...
async function step5Synthesis(ctx) {
    const emit = ctx?.emit;
    const accepted = Array.isArray(ctx?.logic?.accepted) ? ctx.logic.accepted : [];
    if (accepted.length === 0)
        throw new Error("Step 5 failed: no accepted facts");
    await (0, ops_utils_1.pause)(emit, { step: 5, message: "Core synthesis engaged. Weighting facts by model specialty." });
    ...
    await (0, ops_utils_1.pause)(emit, { step: 5, message: "Clustering high-score facts to reduce redundancy." });
    ...
    await (0, ops_utils_1.pause)(emit, { step: 5, message: "Synthesizing consensus narrative and action-oriented output." });
...
exports.default = step5Synthesis;
//# sourceMappingURL=step5_synthesis.js.map