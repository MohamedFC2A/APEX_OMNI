...
import { StepContext, Fact, AgentExecution } from "./types";

function extractTextFromExecution(execution: AgentExecution | undefined): string {
  if (!execution) return "";
...
async function step2Deconstruct(ctx: StepContext): Promise<Fact[]> {
  const emit = ctx?.emit;
  ...
  const perAgentFacts: Array<{ exec: AgentExecution; facts: Array<{ fact: string; words: string[] }> }> = [];
  let totalFacts = 0;
...
export default step2Deconstruct;
