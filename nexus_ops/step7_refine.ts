...
function strengthenVerbs(s: string): string {
...
function enforceActiveVoice(s: string): string {
  return String(s || "")
...
function adjustTone(s: string, userQuery: string | undefined): string {
...
async function step7Refine(ctx: StepContext): Promise<string> {
  const emit = ctx?.emit;
